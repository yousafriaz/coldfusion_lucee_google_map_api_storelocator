...
function initMap() {
    ...
    map.data.loadGeoJson('cfc/locations_.cfc?method=myLocationData', {
        idPropertyName: 'storeid'
    });
    //mapbk =  map.data ; 
    //console.log(mapbk);
    mapbk.data.loadGeoJson('cfc/locations_.cfc?method=myLocationData', {
        idPropertyName: 'storeid'
    });
    ...
    var strprod = 'YOUR_PRODUCTION_WWW_URL';

    // if request is from prod server use key below else use other key for the localhost testing
    if (strhostname.indexOf(strprod) !== -1) {
        var apiKey = 'YOUR_API_KEY_PRODUCTION';
    } else {
    ...
    map.data.addListener('click', (event) => {
        ...
        const content = `
<img style="float:left; width:100px; margin-top:30px" src="img/logo_map.png">
    ...
    autocomplete.addListener('place_changed', async () => {
        ...
        // if there is issue with myCalc function , then we can disable this function
        // and enable rankedStores and update map below , which work fine except 
        // for hawaii locations . The reason why i started using myCalc Instead of rankedStores
        // is because myCalc uses local function to calculate distance between point and bring back
        // 5 closst results - on the other hand rankedStore send complete data set X # of location points to 
        // google matrix api service which can only take 25 destination at a time and for that reason i had to 
        // comeup with sending 25 batch at a time in a loop . so to avoid all that . 
        const myCalc = getGlobal(originLocation, 5);

        /* Substitute for myCalc Start
	...
	*/
    ...
    function selectOpion(selectElm, choice) {
        ...
        $.getJSON({
            url: 'cfc/locations_.cfc?method=myLocationData&' + choice + '=' + p,
            data: {
                sensor: false
            },
            success: function(data, textStatus) {
