<cfset storeids = '41,%2042,%2044,%2043,%2045' >
<cfset nt = 'Request URL: http://127.0.0.1/mygooglemap/cfc/locations_.cfc?method=myLocationData&storeids=41,%2042,%2044,%2043,%2045&strLimit=5&sensor=false'>
<cfset abc = reReplace(urldecode(storeids), "[[:space:]]", "", "ALL") >
<cfdump var="#abc#">


<cfabort>

<cfspreadsheet action="read" src="data.xlsx"  query="myCSV" rows="2-82" headerrow="1" >
<cfset count = 1 >
...
<cfset myQuery = QueryNew("storeid, name, distance , city , state , lat , lng", "Integer ,VarChar, Integer, Varchar , Varchar, Decimal  , Decimal ")> 
<cfset QueryAddRow(MyQuery, #myLocations.recordcount#)>
<cfset count = 0 >
<!--- Output the distance. --->
<cfloop query="#myLocations#">
<cfset count = count + 1 >
...
<cfoutput>
		...
		#GetLatitudeLongitudeProximity(
			...
			LSParseNumber(val(myLocations.lat)),
			LSParseNumber(val(myLocations.lng))
			)# >
		
    
<cfset QuerySetCell(myQuery, "storeid", "#storeid#",#count#)> 
<cfset QuerySetCell(myQuery, "name", "#name#",#count#)> 
<cfset QuerySetCell(myQuery, "distance", "#dd#",#count#)> 
<cfset QuerySetCell(myQuery, "city", "#city#",#count#)> 
<cfset QuerySetCell(myQuery, "state", "#state#",#count#)> 
<cfset QuerySetCell(myQuery, "lat", "#lat#",#count#)> 
<cfset QuerySetCell(myQuery, "lng", "#lng#",#count#)> 
...
--->
 ...
 <cfquery name="distance" dbtype="query">
 	select * from myQuery order by distance asc 
 </cfquery>    
 
 <cfdump var="#distance#">           
            
<!---<cfhttp url="http://127.0.0.1/mygooglemap/data/data.csv"  >
--->
...
<cfset csv_query = CSVToQuery(
	CSV = cfhttp.FileContent.Trim()
	) />
